...
<head>
  ...
  <style>
    ...
    .theme-switch {{ position: relative; width: 92px; height: 38px; border: 0; border-radius: 999px; cursor: pointer; padding: 0; overflow: hidden; appearance: none; -webkit-appearance: none; background: linear-gradient(180deg, #18233f, #0d1326); box-shadow: inset 0 0 0 1px rgba(255,255,255,.09), inset 0 8px 18px rgba(0,0,0,.28), 0 8px 20px rgba(0,0,0,.18); transition: transform .2s ease, background .55s ease, box-shadow .55s ease; }}
    .theme-switch:hover {{ transform: translateY(-1px); }}
    .theme-switch:focus-visible {{ outline: 2px solid var(--link); outline-offset: 2px; }}
    .theme-switch .sky-glow {{ position:absolute; inset:-26px auto auto -20px; width:76px; height:76px; border-radius:50%; background:rgba(255,255,255,.06); box-shadow: 18px 0 0 rgba(255,255,255,.04), 36px 0 0 rgba(255,255,255,.025); transform: translateX(50px) scale(.7); opacity:0; transition: transform .7s cubic-bezier(.56,1.35,.52,1), opacity .45s ease; }}
    .theme-switch .stars {{ position:absolute; inset:0; opacity:1; transition: opacity .35s ease, transform .6s ease; }}
    .theme-switch .stars span {{ position:absolute; width:3px; height:3px; border-radius:50%; background:#fff8c9; box-shadow:0 0 6px rgba(255,248,201,.75); }}
    .theme-switch .stars span:nth-child(1) {{ left:19px; top:9px; }}
    .theme-switch .stars span:nth-child(2) {{ left:36px; top:23px; width:2px; height:2px; }}
    .theme-switch .stars span:nth-child(3) {{ left:59px; top:11px; width:2px; height:2px; }}
    .theme-switch .stars span:nth-child(4) {{ left:74px; top:25px; }}
    .theme-switch .clouds {{ position:absolute; right:-5px; bottom:-8px; width:62px; height:24px; opacity:0; transform: translateY(12px); transition: opacity .4s ease, transform .7s cubic-bezier(.56,1.35,.52,1); }}
    .theme-switch .clouds span {{ position:absolute; bottom:0; border-radius:50%; background:rgba(255,255,255,.92); box-shadow:0 2px 8px rgba(95,132,190,.16); }}
    .theme-switch .clouds span:nth-child(1) {{ right:0; width:26px; height:26px; }}
    .theme-switch .clouds span:nth-child(2) {{ right:17px; width:30px; height:30px; }}
    .theme-switch .clouds span:nth-child(3) {{ right:39px; width:22px; height:22px; }}
    .theme-switch .orb {{ position:absolute; top:5px; left:5px; width:28px; height:28px; border-radius:50%; background:linear-gradient(145deg,#dfe7f6,#8794aa); box-shadow:3px 4px 8px rgba(0,0,0,.34), inset -4px -5px 6px rgba(0,0,0,.18), inset 3px 4px 5px rgba(255,255,255,.28); transition:left .7s cubic-bezier(.56,1.35,.52,1), background .55s ease, box-shadow .55s ease; }}
    .theme-switch .crater {{ position:absolute; border-radius:50%; background:rgba(87,96,116,.55); box-shadow: inset 0 1px 2px rgba(0,0,0,.24); transition: opacity .3s ease; }}
    .theme-switch .crater:nth-child(1) {{ width:7px; height:7px; left:15px; top:6px; }}
    .theme-switch .crater:nth-child(2) {{ width:5px; height:5px; left:7px; top:14px; }}
    .theme-switch .crater:nth-child(3) {{ width:4px; height:4px; left:18px; top:18px; }}
    :root[data-theme='light'] .theme-switch {{ background: linear-gradient(180deg,#69b8f5,#4d91d2); box-shadow: inset 0 0 0 1px rgba(255,255,255,.28), inset 0 8px 18px rgba(255,255,255,.18), 0 8px 20px rgba(62,115,180,.18); }}
    :root[data-theme='light'] .theme-switch .sky-glow {{ transform: translateX(0) scale(1); opacity:1; }}
    :root[data-theme='light'] .theme-switch .stars {{ opacity:0; transform: translateY(12px); }}
    :root[data-theme='light'] .theme-switch .clouds {{ opacity:1; transform: translateY(0); }}
    :root[data-theme='light'] .theme-switch .orb {{ left:59px; background:linear-gradient(145deg,#ffe36a,#ffba25); box-shadow:3px 4px 8px rgba(115,86,18,.28), inset -4px -5px 7px rgba(176,96,0,.22), inset 4px 5px 6px rgba(255,255,255,.42); }}
    :root[data-theme='light'] .theme-switch .crater {{ opacity:0; }}
  ...
  <div class=\"topbar\">
    ...
    <div class=\"top-actions\">
      <button id=\"theme-toggle\" class=\"theme-switch\" type=\"button\" aria-label=\"切换主题\" aria-pressed=\"false\">
        ...
        <span class=\"clouds\"><span></span><span></span><span></span></span>
        <span class=\"orb\"><span class=\"crater\"></span><span class=\"crater\"></span><span class=\"crater\"></span></span>
      </button>
